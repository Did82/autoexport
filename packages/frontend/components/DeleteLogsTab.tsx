...
import { DeleteLog } from '@/types/types';
import { formatDate, humanizeTime } from '@/lib/utils';
import { LogTable } from './LogTable';
import { ru } from 'date-fns/locale';

export function DeleteLogsTab({ logs }: { logs: DeleteLog[] }) {
    const [dirFilter, setDirFilter] = useState('');
    const [dateFilter, setDateFilter] = useState<Date | undefined>(undefined);
    const [page, setPage] = useState(1);
    const [sort, setSort] = useState<{
        field: keyof DeleteLog;
        direction: 'asc' | 'desc';
    ...
    const handleSort = (field: keyof DeleteLog) => {
        setSort((prev) => ({
            field,
            direction:
    ...
    const columns: { key: keyof DeleteLog; label: string }[] = [
        { key: 'createdAt', label: 'Дата и время' },
        { key: 'deletedDir', label: 'Удаленная директория' },
        { key: 'totalTime', label: 'Общее время' },
        { key: 'percentageAfterDelete', label: 'Процент после удаления' },
    ];
